...
| --- | --- | --- |
| `<set-slug>` | Set id from the root `manifest.yaml` (e.g. `psych-beeinflussung-from-de`) or the folder name of the set path (e.g. `psych-beeinflussung` for `sets/de/psych-beeinflussung`) | required |
...
make export ARGS="psych-beeinflussung --format json --out /tmp/review.json"

# Large set: split into parts of at most 10 lessons each for an AI with a
# limited context window
make export ARGS="psych-intro --split-size 10"
# -> exports/psych-intro-de-<timestamp>-part01-of-12.yaml, part02-of-12, ...
```
